...
if st.button("Calculate"):
    ...
    with col1:
        ...
        st.metric("Total Profit", f"${result['profit']:.2f}")
    ...
    revenue_data = {
        "Category": ["Fish Revenue", "Plant Revenue"],
        "Value": [result['fish_revenue'], result['plant_revenue']]
    }
    st.bar_chart(revenue_data, x="Category", y="Value")
    
    # Cost Breakdown
    st.subheader("Cost Breakdown")
    cost_data = {
        "Category": ["Fish Cost", "Plant Cost", "Water Bed Cost", "Feed Cost"],
        "Value": [result['total_fish_cost'], result['total_plant_cost'], 
                  result['total_water_bed_cost'], result['feed_cost']]
    }
    st.bar_chart(cost_data, x="Category", y="Value")
    ...
    st.write(f"Your estimated annual profit is ${result['profit']:.2f}, with a total revenue of ${result['total_revenue']:.2f} and total costs of ${result['total_cost']:.2f}.")
    
    if result['profit'] > 0:
        st.success("Your aquaponics system is profitable!")
    else:
        st.warning("Your aquaponics system is currently not profitable. Consider adjusting your parameters or scaling up your operation.")
